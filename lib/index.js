...
var DocumentOutliner = /** @class */ (function () {
    ...
    DocumentOutliner.prototype.buildHtml = function (options) {
        var html = '';
        var anchor = 1;
        ...
        if (typeof opt.target === 'string') {
            [].forEach.call(document.querySelectorAll(opt.target), function (dom) {
                dom.innerHTML = html;
            });
        }
        else if (opt.target instanceof NodeList) {
            [].forEach.call(opt.target, function (dom) {
                dom.innerHTML = html;
            });
        }
        else {
