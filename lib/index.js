...
var DocumentOutliner = /** @class */ (function () {
    ...
    DocumentOutliner.prototype.makeList = function (target, options) {
        ...
        var output = function (outline, level) {
            if (level > opt.levelLimit) {
                return;
            }
            var noHeading = true;
            outline.getSections().forEach(function (section) {
            ...
            if (!noHeading) {
                var listClassName = opt.listClassName ? " " + opt.linkClassName : '';
                html += "<" + opt.listType + " class=\"level-" + level + listClassName + "\">";
            }
            outline.getSections().forEach(function (section) {
                var heading = section.getHeading();
                if (isHeadingContent(heading)) {
                    var itemClassName = opt.itemClassName ? " class=\"" + opt.itemClassName + "\"" : '';
                    if (opt.link) {
                        var anchorClassName = opt.linkClassName ? " class=\"" + opt.linkClassName + "\"" : '';
                        var anchorName = opt.anchorName.replace(/\$1/, anchor.toString());
                        if (heading.id) {
                ...
                if (section.getSections()) {
                    var nextLevel = noHeading ? level : level + 1;
